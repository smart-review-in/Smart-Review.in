export default {
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/api/get-config")) {
      ...
      // Secrets/config come from env only — never hardcode these.
      // SUPABASE_URL: set via wrangler.jsonc "vars" (not secret, just the project URL).
      // SUPABASE_SERVICE_ROLE_KEY: set via `wrangler secret put SUPABASE_SERVICE_ROLE_KEY`.
      const SUPABASE_URL = (env.SUPABASE_URL || "").replace(/\/+$/, ""); // strip trailing slash
      ...
      try {
        ...
        if (!Array.isArray(phrasesData)) {
          throw new Error("Unexpected phrases response");
        }
        ...
        phrasesData.forEach(row => {
          if (!phrases[row.rating]) return; // guard against unexpected rating values
